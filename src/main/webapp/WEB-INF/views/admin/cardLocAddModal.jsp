...
<div id="uploadModal" class="modal fade upload" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
	...
	function uploadFile() {
		if($('input[name=filename]')[0].files[0] == undefined) {
			...
			$.ajax({
				...
				success: function(r) {
					if(r) {
						alertify.alert('성공', '기기 관리 카드 업데이트가 성공적으로 되었습니다.', function() {location.reload();})
						
					} else {
						alertify.alert('에러', '기기 업데이트 실패. 엑셀파일을 확인해주세요.')
					}
				} 
			})
		}
	...
	function downloadFile() {
		alertify.confirm("양식 다운로드", "다중 업데이트를 위한 양식파일입니다.<br> <b>기기가 등록 되어 있어야하며, 등록 되어 있지 않으면 기기 등록을 먼저 해주세요.</b><br> 양식은 꼭 맞춰야하며, 양식에 있는 코드는 <b style='color:red'>환경설정-공통코드</b>에 있는 코드를<br>입력해주셔야 정상적으로 추가가 됩니다.", function() {
			var options = 'top=0, left=0, width=0, height=0, status=no, menubar=no, toolbar=no, resizable=no';
    		window.open('/file/excel/cardLocExcel', '다운로드', options);
		}, function() {
